...
export const fetchContractPaymentsSummary = createAsyncThunk('auth/fetchContractPaymentsSummary', async ({activeCompany,month,serverModels=null,params=null}) => {
    try {
        const response = await axios.get(`/contracts/contract_payments_summary/?ac=${activeCompany.id}&month=${month}`,
            {   
                params : params,
                headers: {"X-Requested-With": "XMLHttpRequest"}
            }
...
export const fetchContractPaymentsInLease = createAsyncThunk('organization/fetchContractPaymentsInLease', async ({activeCompany,contract_uuid}) => {
    const response = await axios.get(`/contracts/contract_payments/?active_company=${activeCompany.id}&contract_uuid=${contract_uuid}`, {withCredentials: true});
    return response.data;
});
...
export const updateComprehensiveWarningNotice = createAsyncThunk('auth/updateComprehensiveWarningNotice', async ({data=null},{dispatch}) => {
    dispatch(setIsProgress(true));
    try {
        const response = await axios.post(`/contracts/update_comprehensive_warning_notice/`,
            data,
            {
                withCredentials: true,
            }
...
export const fetchTerminationWarningNoticeInformation = createAsyncThunk('auth/fetchTerminationWarningNoticeInformation', async ({activeCompany,contract},{rejectWithValue}) => {
    try {
        const response = await axios.post(`/contracts/termination_warning_notice_information/`, {
            active_company:activeCompany.id,
            contract:contract
        },{ withCredentials: true, });
        return response.data;
...
export const updateTerminationWarningNotice = createAsyncThunk('auth/updateTerminationWarningNotice', async ({data=null},{dispatch}) => {
    dispatch(setIsProgress(true));
    try {
        const response = await axios.post(`/contracts/update_termination_warning_notice/`,
            data,
            {
                withCredentials: true,
            }
...
const contractSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            //fetch termination warning notice information
            .addCase(fetchTerminationWarningNoticeInformation.pending, (state) => {

            })
            .addCase(fetchTerminationWarningNoticeInformation.fulfilled, (state,action) => {
