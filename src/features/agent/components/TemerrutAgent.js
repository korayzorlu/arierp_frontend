...
function TemerrutAgent(props) {
    const {agentTasks,agentRunning} = useSelector((store) => store.agentTask);

    const dispatch = useDispatch();

    const handleRun = () => {
        props.onChangeProps('title', 'Temerrüt Fatura Kesme');
        props.onChangeProps('templateName', 'temerrut-fatura-kesilecekler.xlsx');
        props.onChangeProps('agentName', 'temerrut_fatura_kesme');
        props.onChangeProps('needLF', true);
        dispatch(setAgentDialog(true));
    }

    return (
        <Block text="Temerrüt Fatura Kesme" icon={<NetworkIntelligenceIcon />}  chipLabel="Kredi Risk İzleme" chipColor="mars">
            <Stack spacing={2}>
                <Typography variant='body2' sx={{color: 'text.secondary'}}>
                    Leaseflex üzerinde yapılan temerrüt fatura kesme işlemini otomatikleştirir.
                </Typography>
                <Typography variant='body2' sx={{color: 'text.secondary'}}>
