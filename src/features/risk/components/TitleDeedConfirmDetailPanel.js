...
import { confirmPartner, fetchPartnerInformation } from 'store/slices/partners/partnerSlice';
import { setChangePartnerDialog, setContractPaymentDialog, setDialog, setInstallmentDialog, setPartnerDialog, setTradeTransactionDialog } from 'store/slices/notificationSlice';
...
import { changePartner, setLeasesParams } from 'store/slices/leasing/leaseSlice';
...
function TitleDeedConfirmDetailPanel(props) {
    const {uuid, titleDeedConfirmLeases} = props;
    ...
    const columns = [
        ...
        { field: '-trade_transactions', headerName: 'Cari Ekstre', flex:2,
            renderCell: (params) => {
                ...
                if (isAuto) return null;
                return (
                    <IconButton aria-label='back' onClick={()=>{dispatch(fetchTradeTransactionsInLease({activeCompany,lease_uuid:params.row.id}));dispatch(setTradeTransactionDialog(true))}}>
                        <ReceiptLongIcon/>
                    </IconButton>
                )
            }
    ...
    const handleSubmit = () => {
        dispatch(confirmPartner({data:{uuid}}))
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={titleDeedConfirmLeases.length > 1 ? `${titleDeedConfirmLeases[0].partner} - ${titleDeedConfirmLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            rows={titleDeedConfirmLeases}
            columns={columns}
            ...
            <Dialog
            title="Seçili Kayıtları Onaya Gönder"
...
export default TitleDeedConfirmDetailPanel
