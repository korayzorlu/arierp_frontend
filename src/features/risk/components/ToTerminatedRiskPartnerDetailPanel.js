...
import { setAlert, setCommitteeFormDialog, setComprehensiveWarningNoticeDialog, setContractPaymentDialog, setDialog, setInstallmentDialog, setPartnerDialog, setTerminationWarningNoticeDialog, setTradeTransactionForCustomerDialog, setWarningNoticeDialog } from 'store/slices/notificationSlice';
...
import TradeTransactionForCustomerDialog from 'component/dialog/TradeTransactionForCustomerDialog';
...
function ToTerminatedRiskPartnerDetailPanel(props) {
    ...
    const columns = [
        ...
        { field: '-trade_transactions', headerName: 'Ekstre', flex:2,
            renderCell: (params) => {
                const isAuto = isAutogeneratedRow(params.row)
                if (isAuto) return null;
                return (
                    <Stack direction="row" spacing={1} sx={{alignItems: "center",height:'100%',}}>
                        ...
                        onClick={()=>{dispatch(fetchTradeTransactionsForCustomerInLease({activeCompany,lease_uuid:params.row.id}));dispatch(setTradeTransactionForCustomerDialog(true))}}
                        />
                    </Stack>
                )
            }
        ...
        { field: 'i', headerName: 'Fesih İhtarı', flex: 2, renderCell: (params) => (
                ...
                ?
                    <Stack direction="row" spacing={1} sx={{alignItems: "center",height:'100%',}}>
                        <TableButton
                        icon={<FeedIcon/>}
                        onClick={() => {handleTerminationWarningNoticeDialog(params.row.id,params.row.contract);createFile(params.row.id,params.row.contract)}}
                        />
                    </Stack>
                :
                    null
    ...
    const createFile = async (uuid,contract) => {
        dispatch(setDialog(false));
        try {
            const response = await axios.post('/risk/create_termination_warning_notice_status/',
                {
                    uuid,
                },
                {
    ...
    const handleTerminationWarningNoticeDialog = async (uuid,contract) => {
        setFileUuid(uuid);
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            ...
            />
            <TradeTransactionForCustomerDialog/>
            <Dialog
...
export default ToTerminatedRiskPartnerDetailPanel
