...
import { fetchRiskPartners, fetchAnnualWarnedRiskPartners, setRiskPartnersLoading, setAnnualWarnedRiskPartnersLoading, setAnnualWarnedRiskPartnersParams } from 'store/slices/leasing/riskPartnerSlice';
...
function AnnualWarnedRiskPartners() {
    ...
    const {annualWarnedRiskPartners,annualWarnedRiskPartnersCount,annualWarnedRiskPartnersParams,annualWarnedRiskPartnersLoading,partnerNotesParams} = useSelector((store) => store.riskPartner);
    ...
    // useEffect(() => {
    //     dispatch(setAnnualWarnedRiskPartnersParams({bigger_than_100:true}));
    // }, []);
    ...
    useEffect(() => {
        startTransition(() => {
            dispatch(fetchAnnualWarnedRiskPartners({activeCompany,params:{...annualWarnedRiskPartnersParams,project}}))
        });

        
    }, [activeCompany,annualWarnedRiskPartnersParams,dispatch]);
    ...
    const handleChangeSpecialPartners = async (value) => {
        dispatch(setAnnualWarnedRiskPartnersParams({special:value,barter:false,virman:false}));
    ...
    const handleChangeBarterPartners = async (value) => {
        dispatch(setAnnualWarnedRiskPartnersParams({barter:value,special:false,virman:false}));
    ...
    const handleChangeVirmanPartners = async (value) => {
        dispatch(setAnnualWarnedRiskPartnersParams({virman:value,special:false,barter:false}));
    ...
    const changeProject = (newValue) => {
        setProject(newValue);
        dispatch(setAnnualWarnedRiskPartnersParams({project:newValue}));
    };
    ...
    return (
        <Stack spacing={2}>
            ...
            getRowId={(row) => row.id}
            loading={annualWarnedRiskPartnersLoading}
            customButtons={
                <>
                    {/* <CustomTableButton
                    title="Sözleşme Bazında Excel'e Aktar"
                    onClick={() => {dispatch(setExportDialog(true));dispatch(fetchExportProcess());setExportURL("/risk/export_annual_warned_risk_partners/")}}
                    icon={<DownloadIcon fontSize="small"/>}
                    /> */}
                    {/* <CustomTableButton
                    title="SMS İçin Excel'e Aktar"
                    onClick={() => {dispatch(setExportDialog(true));dispatch(fetchExportProcess());setExportURL("/risk/export_annual_warned_risk_partners_for_sms/")}}
                    ...
                    <CustomTableButton
                    title="Yenile"
                    onClick={() => dispatch(fetchAnnualWarnedRiskPartners({activeCompany,params:{...annualWarnedRiskPartnersParams,project}})).unwrap()}
            ...
            customFiltersLeft={
                <>
                    <FormControl sx={{mr: 2}}>
                        ...
                        disabled={annualWarnedRiskPartnersLoading}
                        >
            ...
            rowCount={annualWarnedRiskPartnersCount}
            setParams={(value) => dispatch(setAnnualWarnedRiskPartnersParams(value))}
            ...
            exportURL={exportURL}
            startEvent={() => dispatch(setAnnualWarnedRiskPartnersLoading(true))}
            finalEvent={() => {dispatch(fetchAnnualWarnedRiskPartners({activeCompany,params:{...annualWarnedRiskPartnersParams,project}}));dispatch(setAnnualWarnedRiskPartnersLoading(false));}}
            project={project}
            />
            <CallDialog/>
            <SendSMSDialog
            risk_status="annual_warned"
            project={project}
...
export default AnnualWarnedRiskPartners
