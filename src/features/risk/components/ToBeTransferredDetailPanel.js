...
function ToBeTransferredDetailPanel(props) {
    const {uuid, toBeTransferredLeases} = props;
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={toBeTransferredLeases.length > 1 ? `${toBeTransferredLeases[0].partner} - ${toBeTransferredLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            rows={toBeTransferredLeases}
            columns={columns}
...
export default ToBeTransferredDetailPanel
