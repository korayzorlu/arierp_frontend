...
function UnderReviewDetailPanel(props) {
    ...
    const columns = [
        ...
        { field: '', headerName: 'Tahsilatlar', flex:2, renderCell: (params) => (
                <IconButton aria-label='back' onClick={()=>{dispatch(fetchContractPaymentsInLease({activeCompany,contract_uuid:params.row.contract_uuid}));dispatch(setContractPaymentDialog(true))}}>
                    <PaidIcon/>
                </IconButton>
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={underReviewLeases.length > 1 ? `${underReviewLeases[0].partner} - ${underReviewLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            //density="compact"
...
export default UnderReviewDetailPanel
