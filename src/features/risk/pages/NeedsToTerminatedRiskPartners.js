...
import { fetchRiskPartners, fetchNeedsToTerminatedRiskPartners, setRiskPartnersLoading, setNeedsToTerminatedRiskPartnersLoading, setNeedsToTerminatedRiskPartnersParams } from 'store/slices/leasing/riskPartnerSlice';
...
import NeedsToTerminatedRiskPartnerDetailPanel from '../components/NeedsToTerminatedRiskPartnerDetailPanel';

function NeedsToTerminatedRiskPartners() {
    const {dark} = useSelector((store) => store.auth);
    const {activeCompany} = useSelector((store) => store.organization);
    const {needsToTerminatedRiskPartners,needsToTerminatedRiskPartnersCount,needsToTerminatedRiskPartnersParams,needsToTerminatedRiskPartnersLoading,partnerNotesParams} = useSelector((store) => store.riskPartner);
    ...
    // useEffect(() => {
    //     dispatch(setNeedsToTerminatedRiskPartnersParams({bigger_than_100:true}));
    // }, []);
    ...
    useEffect(() => {
        startTransition(() => {
            dispatch(fetchNeedsToTerminatedRiskPartners({activeCompany,params:{...needsToTerminatedRiskPartnersParams,project}}));
        });

        
    }, [activeCompany,needsToTerminatedRiskPartnersParams,dispatch]);
    ...
    const handleChangeOverdueTerminatedPartners = async (value) => {
        dispatch(setNeedsToTerminatedRiskPartnersParams({overdue_terminated:value}));
        setOverdueTerminatedSwitchPosition(value);
    };

    const handleChangeSpecialPartners = async (value) => {
        dispatch(setNeedsToTerminatedRiskPartnersParams({special:value,barter:false,virman:false}));
    ...
    const handleChangeBarterPartners = async (value) => {
        dispatch(setNeedsToTerminatedRiskPartnersParams({barter:value,special:false,virman:false}));
    ...
    const handleChangeVirmanPartners = async (value) => {
        dispatch(setNeedsToTerminatedRiskPartnersParams({virman:value,special:false,barter:false}));
    ...
    const changeProject = (newValue) => {
        setProject(newValue);
        dispatch(setNeedsToTerminatedRiskPartnersParams({project:newValue}));
    };
    ...
    return (
        <PanelContent>
            <Grid container spacing={1}>
                <ListTableServer
                title="Üst Üste 2 Defa veya 1 Yılda 3 Defa İhtar Alan ve Fesih Edilmesi Gereken Müşteriler"
                rows={needsToTerminatedRiskPartners}
                columns={riskPartnerColumns}
                getRowId={(row) => row.id}
                loading={needsToTerminatedRiskPartnersLoading}
                customButtons={
                    <>
                        <CustomTableButton
                        title="Excel Hazırla ve İndir"
                        onClick={() => {dispatch(setExportDialog(true));dispatch(fetchExportProcess());setExportURL("/risk/export_needs_to_terminated_risk_partners/")}}
                        icon={<DownloadIcon fontSize="small"/>}
                        />
                        {/* <CustomTableButton
                        title="SMS İçin Excel'e Aktar"
                        onClick={() => {dispatch(setExportDialog(true));dispatch(fetchExportProcess());setExportURL("/risk/export_needs_to_terminated_risk_partners_for_sms/")}}
                        ...
                        <CustomTableButton
                        title="Yenile"
                        onClick={() => dispatch(fetchNeedsToTerminatedRiskPartners({activeCompany,params:{...needsToTerminatedRiskPartnersParams,project}})).unwrap()}
                ...
                customFiltersLeft={
                    <>
                        <FormControl sx={{mr: 2}}>
                            ...
                            disabled={needsToTerminatedRiskPartnersLoading}
                            >
                ...
                rowCount={needsToTerminatedRiskPartnersCount}
                setParams={(value) => dispatch(setNeedsToTerminatedRiskPartnersParams(value))}
                ...
                //onDetailPanelExpandedRowIdsChange={(newExpandedRowIds) => {setDetailPanelExpandedRowIds(new Set(newExpandedRowIds));dispatch(fetchRiskPartners({activeCompany,params:needsToTerminatedRiskPartnersParams}));}}
                getDetailPanelHeight={() => "auto"}
                getDetailPanelContent={(params) => {return(<NeedsToTerminatedRiskPartnerDetailPanel uuid={params.row.uuid} riskPartnerLeases={params.row.leases.leases}></NeedsToTerminatedRiskPartnerDetailPanel>)}}
                />
            </Grid>
            <DeleteDialog
            ...
            startEvent={() => dispatch(setNeedsToTerminatedRiskPartnersLoading(true))}
            finalEvent={() => {dispatch(fetchRiskPartners({activeCompany,params:needsToTerminatedRiskPartnersParams}));dispatch(setNeedsToTerminatedRiskPartnersLoading(false));}}
            />
            <ExportDialog
            handleClose={() => dispatch(setExportDialog(false))}
            exportURL={exportURL}
            startEvent={() => dispatch(setNeedsToTerminatedRiskPartnersLoading(true))}
            finalEvent={() => {dispatch(fetchNeedsToTerminatedRiskPartners({activeCompany,params:{...needsToTerminatedRiskPartnersParams,project}}));dispatch(setNeedsToTerminatedRiskPartnersLoading(false));}}
            project={project}
            />
            <CallDialog/>
            <SendSMSDialog
            risk_status="needs_to_terminated"
            project={project}
...
export default NeedsToTerminatedRiskPartners
