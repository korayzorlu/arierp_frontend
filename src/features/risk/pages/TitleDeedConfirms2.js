...
import { fetchTitleDeedConfirms, setTitleDeedConfirmsLoading, setTitleDeedConfirmsParams } from 'store/slices/leasing/riskPartnerSlice';
...
import { Badge, Button, Chip, FormControl, Grid, IconButton, InputLabel, Menu, MenuItem, NativeSelect, Select, Stack, TextField } from '@mui/material';
...
import TitleDeedConfirmDetailPanel from 'features/risk/components/TitleDeedConfirmDetailPanel';
...
function TitleDeedConfirms() {
    const {dark} = useSelector((store) => store.auth);
    const {activeCompany} = useSelector((store) => store.organization);
    const {titleDeedConfirms,titleDeedConfirmsCount,titleDeedConfirmsParams,titleDeedConfirmsLoading,partnerNotesParams} = useSelector((store) => store.riskPartner);
    ...
    // useEffect(() => {
    //     dispatch(setTitleDeedConfirmsParams({bigger_than_100:true}));
    // }, []);
    ...
    useEffect(() => {
        startTransition(() => {
            //dispatch(fetchProjects({activeCompany,params:titleDeedConfirmsParams}));
            dispatch(fetchTitleDeedConfirms({activeCompany,params:{...titleDeedConfirmsParams,project}}));
        });
        
    }, [activeCompany,titleDeedConfirmsParams,dispatch]);

    const titleDeedConfirmColumns = [
    ...
    const handleChangeSpecialPartners = async (value) => {
        dispatch(setTitleDeedConfirmsParams({special:value,barter:false,virman:false}));
    ...
    const handleChangeBarterPartners = async (value) => {
        dispatch(setTitleDeedConfirmsParams({barter:value,special:false,virman:false}));
    ...
    const handleChangeVirmanPartners = async (value) => {
        dispatch(setTitleDeedConfirmsParams({virman:value,special:false,barter:false}));
    ...
    const changeProject = (databaseTerm) => {
        ...
        dispatch(setTitleDeedConfirmsParams({project:databaseTerm}));
    ...
    const handleChangeBiggerThan100 = async (value) => {
        if(!value){
            dispatch(setTitleDeedConfirmsParams({bigger_than_100:value,overdue_amount:true}));
        }else{
            dispatch(setTitleDeedConfirmsParams({bigger_than_100:value,overdue_amount:false}));
        }
    ...
    return (
        <PanelContent>
            <Grid container spacing={1}>
                ...
                title="Teslim Onay"
                autoHeight
                rows={titleDeedConfirms}
                columns={titleDeedConfirmColumns}
                getRowId={(row) => row.id}
                loading={titleDeedConfirmsLoading}
                customButtons={
                    <>  
                        <CustomTableButton
                        title="İçe Aktar"
                        ...
                        <CustomTableButton
                        title="Yenile"
                        onClick={() => dispatch(fetchTitleDeedConfirms({activeCompany,params:{...titleDeedConfirmsParams,project}})).unwrap()}
                ...
                customFiltersLeft={
                    <>
                        <FormControl sx={{mr: 2}}>
                            ...
                            disabled={titleDeedConfirmsLoading}
                            >
                                {/* {
                                    projects.map((item) => (
                ...
                //onDetailPanelExpandedRowIdsChange={(newExpandedRowIds) => {setDetailPanelExpandedRowIds(new Set(newExpandedRowIds));dispatch(fetchTitleDeedConfirms({activeCompany,params:titleDeedConfirmsParams}));}}
                getDetailPanelHeight={() => "auto"}
                getDetailPanelContent={(params) => {return(<TitleDeedConfirmDetailPanel uuid={params.row.id} titleDeedConfirmLeases={params.row.leases.leases}></TitleDeedConfirmDetailPanel>)}}
                />
            </Grid>
            <DeleteDialog
            ...
            startEvent={() => dispatch(setTitleDeedConfirmsLoading(true))}
            finalEvent={() => {dispatch(fetchTitleDeedConfirms({activeCompany,params:titleDeedConfirmsParams}));dispatch(setTitleDeedConfirmsLoading(false));}}
            />
            <ExportDialog
            handleClose={() => dispatch(setExportDialog(false))}
            exportURL="/leasing/export_title_deed_confirms/"
            startEvent={() => dispatch(setTitleDeedConfirmsLoading(true))}
            finalEvent={() => {dispatch(fetchTitleDeedConfirms({activeCompany,params:{...titleDeedConfirmsParams,project}}));dispatch(setTitleDeedConfirmsLoading(false));}}
            project={project}
            />
            <CallDialog/>
            <MessageDialog/>
...
export default TitleDeedConfirms
