...
import { fetchDepositeToWarnedRiskPartners, fetchRiskPartners, fetchToWarnedRiskPartners, setRiskPartnersLoading, setDepositeToWarnedRiskPartnersLoading, setDepositeToWarnedRiskPartnersParams } from 'store/slices/leasing/riskPartnerSlice';
...
function ToWarnedRiskPartnersDeposit() {
    const {activeCompany} = useSelector((store) => store.organization);
    const {depositeToWarnedRiskPartners,depositeToWarnedRiskPartnersCount,depositeToWarnedRiskPartnersParams,depositeToWarnedRiskPartnersLoading,partnerNotesParams} = useSelector((store) => store.riskPartner);
    ...
    // useEffect(() => {
    //     dispatch(setDepositeToWarnedRiskPartnersParams({bigger_than_100:true}));
    // }, []);
    ...
    useEffect(() => {
        startTransition(() => {
            dispatch(fetchDepositeToWarnedRiskPartners({activeCompany,params:{...depositeToWarnedRiskPartnersParams,project}}))
        });

        
    }, [activeCompany,depositeToWarnedRiskPartnersParams,dispatch]);
    ...
    const handleChangeSpecialPartners = async (value) => {
        dispatch(setDepositeToWarnedRiskPartnersParams({special:value,barter:false,virman:false}));
    ...
    const handleChangeBarterPartners = async (value) => {
        dispatch(setDepositeToWarnedRiskPartnersParams({barter:value,special:false,virman:false}));
    ...
    const handleChangeVirmanPartners = async (value) => {
        dispatch(setDepositeToWarnedRiskPartnersParams({virman:value,special:false,barter:false}));
    ...
    const changeProject = (newValue) => {
        setProject(newValue);
        dispatch(setDepositeToWarnedRiskPartnersParams({project:newValue}));
    };
    ...
    return (
        <PanelContent>
            <Grid container spacing={1}>
                ...
                getRowId={(row) => row.id}
                loading={depositeToWarnedRiskPartnersLoading}
                customButtons={
                    <>
                        <CustomTableButton
                        title="Sözleşme Bazında Excel'e Aktar"
                        onClick={() => {dispatch(setExportDialog(true));dispatch(fetchExportProcess());setExportURL("/risk/export_deposite_to_warned_risk_partners/")}}
                        ...
                        <CustomTableButton
                        title="Yenile"
                        onClick={() => dispatch(fetchDepositeToWarnedRiskPartners({activeCompany,params:{...depositeToWarnedRiskPartnersParams,project,type:'kapora'}})).unwrap()}
                ...
                customFiltersLeft={
                    <>
                        <FormControl sx={{mr: 2}}>
                            ...
                            disabled={depositeToWarnedRiskPartnersLoading}
                            >
                ...
                rowCount={depositeToWarnedRiskPartnersCount}
                setParams={(value) => dispatch(setDepositeToWarnedRiskPartnersParams(value))}
                ...
                //onDetailPanelExpandedRowIdsChange={(newExpandedRowIds) => {setDetailPanelExpandedRowIds(new Set(newExpandedRowIds));dispatch(fetchRiskPartners({activeCompany,params:depositeToWarnedRiskPartnersParams}));}}
            ...
            exportURL={exportURL}
            startEvent={() => dispatch(setDepositeToWarnedRiskPartnersLoading(true))}
            finalEvent={() => {dispatch(fetchDepositeToWarnedRiskPartners({activeCompany,params:{...depositeToWarnedRiskPartnersParams,project}}));dispatch(setDepositeToWarnedRiskPartnersLoading(false));}}
            project={project}
            />
            <CallDialog/>
            <SendSMSDialog
            risk_status="to_warned_deposit"
            project={project}
...
export default ToWarnedRiskPartnersDeposit
