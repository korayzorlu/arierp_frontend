import { Box, Button, Checkbox, Chip, Container, Grid, IconButton, Paper, Stack, Typography } from '@mui/material'
...
import { fetchThirdPersons, fetchVPosThirdPersons, setVPosThirdPersonsLoading, setVPosThirdPersonsParams, updateThirdPerson, updateThirdPersonDowngrade, updateThirdPersonIsEmailSent } from '../../../store/slices/compliance/thirdPersonSlice';
...
import { setImportDialog, setThirdPersonCustomerDialog, setThirdPersonDocumentDialog, setThirdPersonPaymentDetailDialog, setThirdPersonStatusDialog } from 'store/slices/notificationSlice';
...
function VposThirdPersons() {
    const {dark,user} = useSelector((store) => store.auth);
    const {mobile} = useSelector((store) => store.sidebar);
    const {activeCompany} = useSelector((store) => store.organization);
    const {vPosThirdPersons,vPosThirdPersonsCount,vPosThirdPersonsParams,vPosThirdPersonsLoading,thirdPerson} = useSelector((store) => store.thirdPerson);
    ...
    useEffect(() => {
        startTransition(() => {
            dispatch(fetchVPosThirdPersons({activeCompany,params:vPosThirdPersonsParams}));
        });
    }, [activeCompany,vPosThirdPersonsParams,dispatch]);
    ...
    const columns = [
            ...
            { field: 'tc_vkn_no', headerName: 'Kart No', width: 140 },
    ...
    return (
        <PanelContent>
            <ListTableServer
            title="3. Şahıs Ödemelerinde Sorgulanacak Kişiler Listesi (Sanal Postan Gelenler)"
            //autoHeight
            rows={vPosThirdPersons}
            columns={columns}
            getRowId={(row) => row.id}
            loading={vPosThirdPersonsLoading}
            customButtons={
                <> 
                    <CustomTableButton
                    title="İçe Aktar"
                    ...
                    <CustomTableButton
                    title="Yenile"
                    onClick={() => dispatch(fetchVPosThirdPersons({activeCompany,params:vPosThirdPersonsParams})).unwrap()}
                    icon={<RefreshIcon fontSize="small"/>}
                    />
                </>
            }
            rowCount={vPosThirdPersonsCount}
            //checkboxSelection
            disableRowSelectionOnClick
            setParams={(value) => dispatch(setVPosThirdPersonsParams(value))}
            headerFilters={true}
            apiRef={apiRef}
            ...
            row={selectedRow}
            finalEvent={() => dispatch(fetchVPosThirdPersons({activeCompany,params:vPosThirdPersonsParams})).unwrap()}
            />
            ...
            templateURL="/compliance/vpos_third_persons_template"
            importURL="/compliance/import_vpos_third_persons/"
            fileName="sanal-pos-template.xlsx"
            startEvent={() => dispatch(setVPosThirdPersonsLoading(true))}
            finalEvent={() => {dispatch(fetchVPosThirdPersons({activeCompany,params:vPosThirdPersonsParams})).unwrap();dispatch(setVPosThirdPersonsLoading(false));}}
            >
...
export default VposThirdPersons
