...
import { setContractPaymentDialog, setExportDialog, setInstallmentDialog, setPartnerDialog, setTradeTransactionDialog, setTrialBalanceTransactionDialog, setWarningNoticeDialog } from 'store/slices/notificationSlice';
...
import { fetchTrialBalances, setTrialBalancesLoading } from 'store/slices/accounting/trialBalanceSlice';
import { fetchTrialBalanceTransactionsInLease, setTrialBalanceTransactionsLoading } from 'store/slices/accounting/trialBalanceTransactionSlice';
import { setTrialBalanceContractsLoading } from 'store/slices/accounting/trialBalanceContractSlice';
import TrialBalanceTransactionDialog from 'component/dialog/TrialBalanceTransactionDialog';
...
function TrialBalanceContractDetailPanel(props) {
    ...
    const columns = [
        ...
        { field: 'account_code', headerName: 'Hesap Kodu', width: 200, renderCell: (params) => (
    ...
    const handleProfileDialog = async (params,event) => {
        ...
        }else if(params.field==="account_code"){
            dispatch(setTrialBalanceContractsLoading(true));
            await dispatch(fetchTrialBalanceTransactionsInLease({activeCompany,tb_uuid:params.row.id})).unwrap();
            dispatch(setTrialBalanceTransactionDialog(true));
            dispatch(setTrialBalanceContractsLoading(false));
        };
    };
