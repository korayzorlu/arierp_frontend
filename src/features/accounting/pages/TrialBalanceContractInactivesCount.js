...
function TrialBalanceContractInactivesCount() {
    const {user,dark} = useSelector((store) => store.auth);
    const {activeCompany} = useSelector((store) => store.organization);
    const {mainAccountCodesParams,mainAccountCodes} = useSelector((store) => store.trialBalance);
    ...
    }, [activeCompany,mainAccountCodesParams,trialBalanceContractsParams,dispatch]);
    ...
    const columns = [
        ...
        ...mainAccountCodes.map((code) => (
            {field: `code-${code}`, headerName: code, width: 60, renderHeaderFilter: () => null,}
        )),
...
export default TrialBalanceContractInactivesCount
