...
function PartnerAdvanceDetailPanel(props) {
    const {uuid, partnerAdvanceLeases} = props;
    ...
    const columns = [
        ...
        { field: '', headerName: 'Tahsilatlar', flex:2,
            renderCell: (params) => {
                ...
                return (
                    <IconButton aria-label='back' onClick={()=>{dispatch(fetchContractPaymentsInLease({activeCompany,contract_id:params.row.contract_id}));dispatch(setContractPaymentDialog(true))}}>
                        <PaidIcon/>
                    </IconButton>
                )
            }
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={partnerAdvanceLeases.length > 1 ? `${partnerAdvanceLeases[0].partner} - ${partnerAdvanceLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            //density="compact"
            rows={partnerAdvanceLeases}
            columns={columns}
...
export default PartnerAdvanceDetailPanel
