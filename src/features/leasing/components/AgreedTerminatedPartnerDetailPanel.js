...
function AgreedTerminatedPartnerDetailPanel(props) {
    const {uuid, agreedTerminatedPartnerLeases} = props;
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={agreedTerminatedPartnerLeases.length > 1 ? `${agreedTerminatedPartnerLeases[0].partner} - ${agreedTerminatedPartnerLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            rows={agreedTerminatedPartnerLeases}
            columns={columns}
...
export default AgreedTerminatedPartnerDetailPanel
