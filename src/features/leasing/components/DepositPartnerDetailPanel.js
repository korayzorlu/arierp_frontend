...
function DepositPartnerDetailPanel(props) {
    const {uuid, depositPartnerLeases} = props;
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={depositPartnerLeases.length > 1 ? `${depositPartnerLeases[0].partner} - ${depositPartnerLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            rows={depositPartnerLeases}
            columns={columns}
...
export default DepositPartnerDetailPanel
