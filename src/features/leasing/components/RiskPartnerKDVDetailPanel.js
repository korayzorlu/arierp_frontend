...
function RiskPartnerKDVDetailPanel(props) {
    const {uuid, riskPartnerKDVLeases} = props;
    ...
    return (
        <Box sx={{ pt: 2, pb: 2, pl: 8, pr: 8 }}>
            <ListTable
            title={riskPartnerKDVLeases.length > 1 ? `${riskPartnerKDVLeases[0].partner} - ${riskPartnerKDVLeases[0].partner_tc} Kira Planları` : ""}
            height="auto"
            autoHeight
            rows={riskPartnerKDVLeases}
            columns={columns}
...
export default RiskPartnerKDVDetailPanel
