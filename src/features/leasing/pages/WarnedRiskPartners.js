...
import { fetchRiskPartners, fetchWarnedRiskPartners, setWarnedRiskPartnersLoading, setWarnedRiskPartnersParams } from '../../../store/slices/leasing/riskPartnerSlice';
...
function WarnedRiskPartners() {
    ...
    useEffect(() => {
        startTransition(() => {
            dispatch(fetchWarnedRiskPartners({activeCompany,params:warnedRiskPartnersParams}));
        });
    ...
    const handleChangeBiggerThan100 = async (value) => {
        if(!value){
            dispatch(setWarnedRiskPartnersParams({bigger_than_100:value,overdue_amount:true}));
        }else{
            dispatch(setWarnedRiskPartnersParams({bigger_than_100:value,overdue_amount:false}));
        }
            ...
            <Grid container spacing={1}>
                ...
                customButtons={
                    <>
                        ...
                        <CustomTableButton
                        title="Yenile"
                        onClick={() => dispatch(fetchRiskPartners({activeCompany,params:warnedRiskPartnersParams})).unwrap()}
